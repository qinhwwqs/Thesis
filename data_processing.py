...
def TrainImages(n_set,n_flights ,filter_t,f_modes):
  ...
  set = 'C:/Users/andre/Desktop/论文/data/C-MAPSS/train_FD00'+str(n_set)+'.txt'
  ...
  for i in range(1, n_flights + 1):  
      flight = data[data['Unit Number'] == i]  
      RUL = flight['RUL'].to_frame()
      if f_modes:
          X = flight.drop(
              ['Unit Number',
...
def TestImages(n_set,n_flights ,filter_t,f_modes):
  ...
  images = []
  set_images = C:/Users/andre/Desktop/论文/data/C-MAPSS/test_FD00'+str(n_set)+'.txt'
  set_rul = 'C:/Users/andre/Desktop/论文/data/C-MAPSS/RUL_FD00'+str(n_set)+'.txt'
  ...
  images = np.reshape(images, (images.shape[0], images.shape[1] * images.shape[2]))
  rul = np.array(pd.read_csv(set_rul,delim_whitespace=True,header=None))
  if n_flights == 100:
      n_units=100
  elif n_flights == 248:
      n_units = 248
