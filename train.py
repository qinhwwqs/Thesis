...
class model:
            ...
            for step in range(1, self._training_epoch + 1):
                ...
                if self._save == True:
                    ...
                    if step == self._training_epoch:
                        ...
                        if self._arq == 0:
                            prediction = self._model_path + "Conv" + str(self._kind) + "/adjust.txt"

                        else:
                            prediction = self._model_path + "Conv" + str(self._kind) + "_ED/adjust.txt"

                        ajuste =  np.column_stack((batch_y, pred))
                        np.savetxt(prediction, ajuste, delimiter=" ")
    ...
    def test(self):
        ...
        loss_op = tf.reduce_sum(tf.square(Y - pred1)) + tf.reduce_sum(tf.abs(WF)) + 100 * tf.reduce_sum(tf.square(tf.nn.relu(pred1 - 125)))
    ...
    def test_b(self):
        ...
        with tf.Session() as sess:
            ...
            if self._arq == 0:
                prediction = self._model_path + "Conv" + str(self._kind) + "/prediction.txt"
                scort = self._model_path + "Conv" + str(self._kind) + "/score.txt"
                rms = self._model_path + "Conv" + str(self._kind) + "/RMSE.txt"


            else:
                prediction = self._model_path + "Conv" + str(self._kind) + "_ED/prediction.txt"
            ...
            np.savetxt(prediction, rmse, delimiter=" ")
    ...
    def cross(self):
        ...
        with tf.Session() as sess:
            ...
            print("Testing Accuracy:", sess.run(accuracy, feed_dict={X: batch_x_crossval, Y: batch_y_crossval}))
            stop = time.time()
            t = stop - start
            print('\n testing cross= %g [s] \n' % t)
